...
import "../styles/components_css/LikeBtn.css";
...
const LikeBtn: React.FC<LikeBtnProps> = ({
  ...
  useEffect(() => {
    // Check if the userid is in the likes array for this specific component
    const componentLiked = likedComponents.find((comp) => comp._id === compID);

    if (userid !== null) {
      const isLiked = componentLiked?.likes.includes(userid) ?? false;
      setLiked(isLiked);
    }
  }, [likedComponents, userid, compID]);
  // ... useEffect to check initial state if needed
